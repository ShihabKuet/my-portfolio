...
function FeaturedCard({ project, index }: { project: (typeof projects)[0]; index: number }) {
  return (
    <motion.div
      ...
      className="group relative grid md:grid-cols-2 gap-6 p-6 rounded-2xl bg-white dark:bg-zinc-900/50 border border-sky-200 dark:border-zinc-800/50 hover:border-violet-500/30 transition-all duration-300 hover:shadow-xl hover:shadow-violet-500/5"
    >
      ...
      <div className="relative w-full rounded-xl overflow-hidden border border-zinc-700/30 bg-sky-100 dark:bg-zinc-800/50">
...
{/* ----  className attributes described below for image  ----
    ...
    object-fill:    Stretches to fill — distorts the image, avoid this */}
            ...
            <div className="absolute inset-0 flex items-center justify-center text-center text-sky-400 dark:text-zinc-600">
              <div>
      ...
      <div className="flex flex-col justify-between">
        <div>
          ...
          <h3 className="text-sky-950 dark:text-zinc-100 font-bold text-xl mb-3 group-hover:text-violet-300 transition-colors">
            {project.title}
          </h3>
        ...
        <div className="flex items-center gap-4">
          {project.github && (
            <a href={project.github} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sky-700 dark:text-zinc-400 hover:text-violet-400 transition-colors text-sm font-medium">
              <FaGithub size={16} />
              Source Code
            </a>
          )}
          {project.live && (
            <a href={project.live} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sky-700 dark:text-zinc-400 hover:text-violet-400 transition-colors text-sm font-medium">
...
function ProjectCard({ project, index }: { project: (typeof projects)[0]; index: number }) {
  return (
    <motion.div
      ...
      className="group flex flex-col justify-between p-5 rounded-xl bg-white dark:bg-zinc-900/50 border border-sky-200 dark:border-zinc-800/50 hover:border-violet-500/30 hover:shadow-lg hover:shadow-violet-500/5 transition-all duration-300"
    >
      {/* Top row */}
      <div>
        <div className="flex items-start justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            {project.github && (
              <a href={project.github} target="_blank" rel="noopener noreferrer" aria-label="GitHub" className="text-sky-500 dark:text-zinc-500 hover:text-sky-900 dark:text-zinc-200 transition-colors">
                <FaGithub size={17} />
              </a>
            )}
            {project.live && (
              <a href={project.live} target="_blank" rel="noopener noreferrer" aria-label="Live demo" className="text-sky-500 dark:text-zinc-500 hover:text-sky-900 dark:text-zinc-200 transition-colors">
        ...
        <h3 className="text-sky-950 dark:text-zinc-100 font-semibold text-base mb-2 group-hover:text-violet-300 transition-colors">
          {project.title}
        </h3>
      ...
      {/* Tech tags at bottom */}
      <div className="flex flex-wrap gap-1.5 mt-auto pt-4 border-t border-sky-200 dark:border-zinc-800/50">
        {project.technologies.map((tech) => (
          <TechBadge key={tech} name={tech} />
        ))}
      </div>
    ...
    <section id="projects" className="py-24 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
          <h3 className="text-sky-800 dark:text-zinc-300 font-semibold text-lg">Other Projects</h3>
